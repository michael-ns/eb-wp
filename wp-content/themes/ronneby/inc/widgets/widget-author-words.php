...
class dfd_author_words extends SB_WP_Widget {
    ...
    function widget( $args, $instance ) {
		...
		/*global $dfd_sunday;
		if(isset($dfd_sunday['soc_icons_hover_style']) && !empty($dfd_sunday['soc_icons_hover_style'])) {
			$soc_icons_hover_style = 'dfd-soc-icons-hover-style-'.$dfd_sunday['soc_icons_hover_style'];
		} else {
		...
		<div class="author-image">
			<?php
				if (has_post_thumbnail()) {
					...
					$article_image = dfd_aq_resize($img_url, 80, 80, true, true, true);
					if(!$article_image) {
		...
		<?php
			...
			foreach($soc_networks as $key => $value) {
				...
				if(!empty($soc_account)) {
					$output .= '<a href="'.$soc_account.'" class="'.$value.'"><span class="line-top-left '.esc_attr($value).'"></span><span class="line-top-center '.esc_attr($value).'"></span><span class="line-top-right '.esc_attr($value).'"></span><span class="line-bottom-left '.esc_attr($value).'"></span><span class="line-bottom-center '.esc_attr($value).'"></span><span class="line-bottom-right '.esc_attr($value).'"></span><i class="'.esc_attr($value).'"></i></a>';
				}
			}
