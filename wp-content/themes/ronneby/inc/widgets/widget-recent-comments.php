...
class dfd_recent_comments extends SB_WP_Widget {
	...
	$title = $this->getInstance('title');
	    ...
	    foreach ( (array) $comments as $comment) {
		    ...
		    $d = "F j, Y";
		    $output .=  '<li class="recentcomments">' . /* translators: comments widget: 1: comment author, 2: post link */ sprintf(_x('%1$s on %2$s', 'dfd'), '<p class="comment-text">'.$comment_text.'</p><p class="comment-text"><span class="author">' . get_comment_author_link() . '</span>', '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '" class="dfd-post-link" title=" " targer="_blank">' . get_the_title($comment->comment_post_ID) . '</a></p>' . '<p class="entry-meta"><span class="comments-date">' . get_comment_date($d, $comment->comment_ID)) . '</span></p></li>';
	    }
        }
