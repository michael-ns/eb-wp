...
if(!class_exists('Dfd_Services_Module'))
{
	class Dfd_Services_Module
	{
		...
		function __construct()
		{
			...
			//$this->border_col = '';
			$this->icon_style = '';
			add_action('admin_init', array($this, 'add_dfd_service'));
	...
	// Shortcode Functions for frontend editor
		...
		{
			// Do nothing
			$position = $columns_width = $columns_offsets = $style = $icon_color = $icon_bg_color = $font_size_icon = $icon_border_style = $icon_border_size = $connector_color = $connector_style = $border_color = $el_class = '';
			extract(shortcode_atts(array(
				'position' => 'icon-left',
				'columns_width' => 'full-width-elements',
				//'columns_offsets' => '',
