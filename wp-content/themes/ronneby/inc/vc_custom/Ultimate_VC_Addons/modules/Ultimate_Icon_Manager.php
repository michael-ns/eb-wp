			...
			if(is_array($options) && !empty($options)){
				foreach($options as $key => $opts){
					...
					$output .= '<div class="onoffswitch">
							<input type="checkbox" name="'.esc_attr($param_name).'[]" value="'.esc_attr($key).'" '.$dependency.' class="'.esc_attr($dependency).' onoffswitch-checkbox chk-switch-'.esc_attr($un).'" id="switch'.esc_attr($uid).'" '.$checked.'>
...
';
		...
		function datetimepicker($settings, $value)
		{
			...
			$output = '<div id="ult-date-time'.esc_attr($uni).'" class="ult-datetime"><input data-format="yyyy/MM/dd hh:mm:ss" class="wpb_vc_param_value ' . esc_attr($param_name) . ' ' . esc_attr($type) . ' ' . esc_attr($class) . '" name="' . esc_attr($param_name) . '" style="width:258px;" value="'.esc_attr($value).'" '.$dependency.'/><div class="add-on" >  <i data-time-icon="Defaults-time" data-date-icon="Defaults-time"></i></div></div>';
			$output .= '<script type="text/javascript">
				jQuery(document).ready(function(){
		...
		function icon_manager($settings, $value)
		{
			...
			$output = '<div class="my_param_block">'
					 .'<input name="'.esc_attr($settings['param_name']).'"
					  ...
					  value="'.esc_attr($value).'" ' . $dependency . '/>'
					 .'</div>';
			$output .= '<script type="text/javascript">
				jQuery(document).ready(function(){
