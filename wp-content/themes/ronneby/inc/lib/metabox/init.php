    ...
    function show() {
        ...
        foreach ( $this->_meta_box['fields'] as $field ) {
            ...
            switch ( $field['type'] ) {
                ...
                case 'text':
                    echo '<input type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? $meta : $field['std'], '" />','<p class="cmb_metabox_description">', $field['desc'], '</p>';
                    break;
                case 'icon':
                    echo '<input type="text" class="iconname" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? $meta : $field['std'], '" /> <a href="#" class="button crum-icon-add" title="'.__('Add Icon', 'dfd').'">'.__('Add Icon', 'dfd').'</a>','<p class="cmb_metabox_description">', $field['desc'], '</p>';
                    break;
                case 'sidebar_select':
					...
					echo "<script>
						(function($) {
								...
								$(document).ready(function() {
									var select = $('#dfd_headers_header_style');
									var showHideElements = select.parents('tr').siblings('tr');
									var showHideHeaderOptions = function() {
										if(select.val() != '0' && select.val() != '1'&& select.val() != '2') {
											showHideElements.hide();
										} else {
											showHideElements.show();
										}
										if(select.val() != '2' && select.val() != '4') {
											$('#adaptive-header-options').hide();
										} else {
											$('#adaptive-header-options').show();
										}
									};
                ...
                case 'logo_position_select':                    
    
                    $logo_position = dfd_logo_position();
                    if( is_array($logo_position) ){
                        $select_str = __('-- Select logo position --', 'dfd');
                        array_unshift( $logo_position, $select_str );
                    }

                    if( empty( $meta ) && !empty( $field['std'] ) ) $meta = $field['std'];

                    echo '<select name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '">';
                    foreach ($logo_position as $key => $value) {
                    ...
                    break;
                case 'menu_position_select':                    
    
                    $menu_position = dfd_menu_position();
                    if( is_array($menu_position) ){
                        $select_str = __('-- Select menu position --', 'dfd');
                        array_unshift( $menu_position, $select_str );
                    }

                    if( empty( $meta ) && !empty( $field['std'] ) ) $meta = $field['std'];

                    echo '<select name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '">';
                    foreach ($menu_position as $key => $value) {
                        echo '<option value="', esc_attr($key) , '"', $meta == $key ? ' selected="selected"' : '', '>', $value, '</option>';
                    }
                    echo '</select>';
                    echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
                ...
                case 'text_small':
                    echo '<input class="cmb_text_small" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? $meta : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
                    break;
                case 'text_medium':
                    echo '<input class="cmb_text_medium" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? $meta : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
                    break;
                case 'text_date':
                    echo '<input class="cmb_text_small cmb_datepicker" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? $meta : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
                    break;
                case 'text_date_timestamp':
                    echo '<input class="cmb_text_small cmb_datepicker" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? date( 'm\/d\/Y', $meta ) : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
                    break;

                case 'text_datetime_timestamp':
                    echo '<input class="cmb_text_small cmb_datepicker" type="text" name="', esc_attr($field['id']), '[date]" id="', esc_attr($field['id']), '_date" value="', '' !== $meta ? date( 'm\/d\/Y', $meta ) : $field['std'], '" />';
                    echo '<input class="cmb_timepicker text_time" type="text" name="', esc_attr($field['id']), '[time]" id="', esc_attr($field['id']), '_time" value="', '' !== $meta ? date( 'h:i A', $meta ) : $field['std'], '" /><span class="cmb_metabox_description" >', $field['desc'], '</span>';
                    break;
                case 'text_time':
                    echo '<input class="cmb_timepicker text_time" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? $meta : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
                    break;
                case 'text_money':
                    echo '$ <input class="cmb_text_money" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? $meta : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
                    break;
                case 'colorpicker':
                    ...
                    break;
                case 'textarea':
                    echo '<textarea name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" cols="60" rows="10">', '' !== $meta ? $meta : $field['std'], '</textarea>','<p class="cmb_metabox_description">', $field['desc'], '</p>';
                    break;
                case 'textarea_small':
                    echo '<textarea name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" cols="60" rows="4">', '' !== $meta ? $meta : $field['std'], '</textarea>','<p class="cmb_metabox_description">', $field['desc'], '</p>';
                    break;
                case 'textarea_code':
                    echo '<textarea name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" cols="60" rows="10" class="cmb_textarea_code">', '' !== $meta ? $meta : $field['std'], '</textarea>','<p class="cmb_metabox_description">', $field['desc'], '</p>';
                    break;
                case 'select':
                    if( empty( $meta ) && !empty( $field['std'] ) ) $meta = $field['std'];
                    echo '<select name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '">';
                ...
                case 'radio_inline':
                    ...
                    $i = 1;
                    foreach ($field['options'] as $option) {
                        echo '<div class="cmb_radio_inline_option"><input type="radio" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), esc_attr($i), '" value="', esc_attr($option['value']), '"', $meta == $option['value'] ? ' checked="checked"' : '', ' /><label for="', esc_attr($field['id']), esc_attr($i), '">', $option['name'], '</label></div>';
                        $i++;
                    }
                    echo '</div>';
                    echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
                ...
                case 'radio':
                    ...
                    $i = 1;
                    foreach ($field['options'] as $option) {
                        echo '<li><input type="radio" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), esc_attr($i),'" value="', esc_attr($option['value']), '"', $meta == $option['value'] ? ' checked="checked"' : '', ' /><label for="', esc_attr($field['id']), esc_attr($i), '">', $option['name'].'</label></li>';
                        $i++;
                    }
                    echo '</ul>';
                    echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
                ...
                case 'multicheck':
                    ...
                    foreach ( $field['options'] as $value => $name ) {
                        ...
                        echo '<li><input type="checkbox" name="', esc_attr($field['id']), '[]" id="', esc_attr($field['id']), esc_attr($i), '" value="', esc_attr($value), '"', in_array( $value, $meta ) ? ' checked="checked"' : '', ' /><label for="', esc_attr($field['id']), esc_attr($i), '">', $name, '</label></li>';
                        $i++;
                    }
                    echo '</ul>';
                    echo '<span class="cmb_metabox_description">', $field['desc'], '</span>';
                ...
                case 'taxonomy_select':
                    ...
                    foreach ( $terms as $term ) {
                        if (!is_wp_error( $names ) && !empty( $names ) && !strcmp( $term->slug, $names[0]->slug ) ) {
                            ...
                            echo '<option value="' . esc_attr($term->slug) . '  ' , $meta == $term->slug ? $meta : ' ' ,'  ">' . $term->name . '</option>';
                        }
                    }
                    echo '</select>';
                    echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
                ...
                case 'file':
                    ...
                    if ( 'url' == $field['allow'] || ( is_array( $field['allow'] ) && in_array( 'url', $field['allow'] ) ) )
                        $input_type_url="text";
                    echo '<input class="cmb_upload_file" type="' . esc_attr($input_type_url) . '" size="45" id="', esc_attr($field['id']), '" name="', esc_attr($field['id']), '" value="', $meta, '" />';
                    echo '<input class="cmb_upload_button button" type="button" value="Upload File" />';
                    echo '<input class="cmb_upload_file_id" type="hidden" id="', esc_attr($field['id']), '_id" name="', esc_attr($field['id']), '_id" value="', get_post_meta( $post->ID, $field['id'] . "_id",true), '" />';
                    echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
...
if (!function_exists('dfd_kadabra_cmb_scripts')) {
	function dfd_kadabra_cmb_scripts( $hook ) {
		global $wp_version;
...
add_action( 'admin_enqueue_scripts', 'dfd_kadabra_cmb_scripts', 10 );
