...
<article class="post hnews hentry small-news post post-<?php the_ID(); ?> <?php echo 'format-' . $format ?>">
	<?php if($dfd_ronneby['post_header']) : ?>
		<div class="clearfix">
		...
		switch(true) {
			...
			default:
				...
				if (has_post_thumbnail()) {
					if (!$dfd_ronneby['thumb_image_crop']) {
						$image_crop = true;
					} else {
	...
	<?php if(!has_post_format('quote')) : ?>
		<div class="clearfix">
			<div class="entry-content grid">
				...
				<a href="<?php echo the_permalink(); ?>" title="<?php the_title(); ?>" class="more-button <?php echo $dfd_ronneby['style_hover_read_more']; ?> left" data-lang="en"><?php _e('Continue', 'dfd'); ?></a>
