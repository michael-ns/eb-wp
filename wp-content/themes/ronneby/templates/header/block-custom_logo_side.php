...
if (isset($dfd_ronneby['custom_logo_image_side']['url']) && !empty($dfd_ronneby['custom_logo_image_side']['url'])) :
	...
	if (
		...
		//file_exists($dfd_ronneby['custom_retina_logo_image_side']['url']) &&
		ini_get('allow_url_fopen') && 
		getimagesize($dfd_ronneby['custom_retina_logo_image_side']['url'])
	)
	{
		# Retina ready logo
		$custom_retina_logo_image = dfd_aq_resize($dfd_ronneby['custom_retina_logo_image_side']['url'], $_logo_size[0]*2, $_logo_size[1]*2);
		if(!$custom_retina_logo_image) $custom_retina_logo_image = $dfd_ronneby['custom_retina_logo_image_side']['url'];
		list($logo_image_w, $logo_image_h) = getimagesize($dfd_ronneby['custom_retina_logo_image_side']['url']);
	}
