(function ($) {
	...
	$(document).ready(function() {
		...
		var windowWidth, windowHeight, documentHeight, offsetTop, scrollTop, $window = $(window);
		
		var recalcValues = function() {
			windowWidth = $window.width();
			windowHeight = $window.height();
			documentHeight = $('body').height();
			offsetTop = documentHeight - windowHeight * 1.3;
		};
		...
		$window.bind('scroll', function(e) {
			...
			if (page_num <= max_pages && !$popup.hasClass('visible') && scrollTop < documentHeight && scrollTop > offsetTop && !$popup.hasClass('last-page')) {
				$.ajax({
					type: 'GET',
					url: next_link,
					dataType: 'html',
